                ...
                <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5">
                    <div class="min-w-0 flex-1">
                        ...
                        @if(!empty($notice->reason_text))
                            <p class="mt-3 text-sm leading-7 text-gray-600 whitespace-pre-line">
                                {{ $notice->reason_text }}
                            </p>
                        @endif
                    </div>
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="rounded-2xl bg-gray-50 px-4 py-4">
                                    ...
                                    <div class="text-lg font-bold text-gray-800 break-words">
                                        {{ $item->customer_name }}
                                    </div>
                                    ...
                                    <div class="mt-4 space-y-3 text-sm">
                                        <div>
                                            ...
                                            <div class="font-semibold text-gray-800">{{ $reservationAt ?: '―' }}</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500">メールアドレス</div>
                                            <div class="font-semibold text-gray-800 break-all">{{ $item->customer_email ?: '未登録' }}</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500">電話番号</div>
                                            <div class="font-semibold text-gray-800">{{ $item->customer_phone ?: '未登録' }}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="rounded-2xl bg-gray-50 px-4 py-4">
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="rounded-xl bg-white px-3 py-3 border">
                                        ...
                                        <div class="rounded-xl bg-white px-3 py-3 border col-span-2">
                                            <div class="text-[11px] text-gray-500">確認日時</div>
                                            <div class="text-sm font-bold text-gray-800 mt-1">{{ $confirmedAt ?: '未確認' }}</div>
                                        </div>
                                    </div>

                                    @if(!in_array($item->response_status, ['closed', 'confirmed', 'phone_confirmed'], true))
                                        <form method="POST"
                                        ...
                                        <p class="text-xs text-gray-400 mt-2">
                                            メール送信済みの顧客でも、電話で確認できた場合はこちらで完了にできます
                                        </p>
                                    @endif
                                </div>
                            </div>
