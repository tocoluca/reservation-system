...
<div class="staff-shift-view-page max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <div class="shift-desktop-layout hidden lg:block bg-white border border-gray-200 rounded-3xl shadow-sm p-5 mb-6">
    ...
    <div class="shift-mobile-layout lg:hidden space-y-3">
            ...
            <div id="mobileShiftDateStrip" class="mt-3 flex gap-1.5 overflow-x-auto pb-1 snap-x">
                @for($d = 1; $d <= $days; $d++)
                    ...
                    <button type="button"
                            ...
                            aria-selected="false">
                        {{ $d }}（{{ $weekdayLabels[$mobileDayOfWeek] }}）
                    </button>
                @endfor
            </div>
        </div>
            ...
            <table class="min-w-max text-sm w-full border-collapse">
                <thead>
                    <tr style="background: {{ $theme }}; color: white;">
                        <th class="sticky top-0 left-0 z-40 min-w-[240px] border-r border-b bg-white px-3 py-1.5 text-xs leading-tight text-black">スタッフ</th>
                        ...
                        @for($d = 1; $d <= $days; $d++)
                            ...
                            @endphp
                            <th class="sticky top-0 z-30 min-w-[110px] whitespace-nowrap border-b border-r border-gray-200 px-2 py-1.5 text-center text-xs font-bold leading-none {{ $color }}">
                ...
                <tbody>
                    @foreach($staffs as $staff)
                        ...
                        <tr class="border-b {{ $isTop ? 'bg-amber-50' : 'hover:bg-gray-50' }}">
                            <td class="p-3 sticky left-0 z-20 min-w-[240px] border-r align-top {{ $isTop ? 'bg-amber-100' : 'bg-white' }}">
                                <div class="space-y-2">
